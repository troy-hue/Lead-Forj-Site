      ...
      <div className="relative flex flex-col items-center gap-0 mb-12">
        {layers.map((layer, i) => (
          ...
          >
            {/* Layer bar */}
            <div
              className="relative px-4 md:px-5 py-3 transition-all duration-300 group-hover:brightness-110"
              style={{
                ...
                ...(isMobile && i >= 3 ? { paddingTop: '1rem', paddingBottom: '1rem' } : {}),
              }}
            >
              <div className="flex items-center justify-between">
                <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4 min-w-0">
                  <span className="text-[10px] font-mono tracking-widest whitespace-nowrap" style={{ color: layer.color }}>
                    {layer.label}
                  </span>
                  <span className="text-xs md:text-sm font-medium text-[#F6F3EC]">{layer.title}</span>
                </div>
                {/* Expand indicator */}
                <motion.div
                  animate={{ rotate: activeLayer === layer.id ? 45 : 0 }}
                  transition={{ duration: 0.2 }}
                  className="w-4 h-4 flex items-center justify-center opacity-40 group-hover:opacity-80 flex-shrink-0 ml-1"
